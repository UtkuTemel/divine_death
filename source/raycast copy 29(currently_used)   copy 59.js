    ...
    // --- 3. GAMEPLAY INPUTS ---
        ...
        if (playerInventory.isOpen) {
            if (e.code === "KeyI" || e.code === "Escape") {
                ...
                useSelectedItem(); // This allows using potions
            }
...
function renderChestInterface() {
    ...
    items.forEach((item, index) => {
        ...
        // 1. If it's a Weapon (uses the new 'icon' property)
        if (item.icon) {
            imgToDraw = item.icon;
        } 
        // 2. If it's a Standard Item (uses the global itemImages lookup)
        else if (itemImages[item.type]) {
            imgToDraw = itemImages[item.type];
        }
