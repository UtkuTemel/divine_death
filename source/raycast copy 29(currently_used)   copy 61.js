...
const sword = createWeapon({
    ...
    frameRate: 20,
    damageFrame: 5,
    cooldownDuration: 400, // 600ms cooldown
...
window.addEventListener("keydown", e => {
    ...
    if (gameState === GAME_STATES.MENU) {
        ...
        if (playerInventory.isOpen) {
            ...
            if (e.code === "KeyF") {
                // 1. Try interacting with Items/Doors first
                const handledEnvInteraction = checkItemProximity(); 
                
                // 2. If no item/door interaction, try interacting with NPC
                if (!handledEnvInteraction) {
                    checkNPCInteraction(); // Updates nearbyNPC
                    ...
                    if (nearbyNPC) {
                        ...
                        // Handle "One time" triggers (Library cutscene)
        ...
        if (e.code === "KeyF" || e.code === "Enter" || e.code === "Space") {
            ...
            // Determine which text array to use
            let dialogueSet = [];
            if (inLibraryCutscene) {
                // Special case for library text flow
...
document.addEventListener("mousemove", (e) => {
    if (gameState === GAME_STATES.PLAYING && document.pointerLockElement === canvas) {
...
document.addEventListener("mousedown", (e) => {
    ...
    if (gameState !== GAME_STATES.PLAYING || playerInventory.isOpen) return;
...
function castRays() {
    ...
    for (let i = 0; i < NUM_RAYS; i++) {
        ...
        while (!hitFullWall && distAccumulator < MAX_DEPTH / TILE_SIZE) {
            let side = 0;
...
function render3D(rays) {
    ...
    rays.forEach((ray, i) => {
        ...
        for (let j = ray.steps.length - 1; j >= 0; j--) {
            ...
            if (hit.distance >= MAX_DEPTH) continue;
            ...
            let brightness = MIN_BRIGHTNESS +
                (MAX_BRIGHTNESS - MIN_BRIGHTNESS) * (1 - Math.min(1, correctedDistance / MAX_LIGHT_DISTANCE));
...
function renderSpriteLayer() {
    ...
    renderList.forEach(item => {
        ...
        // We share the same visibility logic for all types
        let isVisible = true;
        // Reduce check steps for items/objects slightly for performance
        ...
        for (let i = 1; i <= checkSteps; i++) {
            ...
            // Check wall collision (ignore 0 and 3)
            if (mapY >= 0 && mapY < map.length && 
        ...
        // Specific logic for Items (usually smaller)
        else if (item.type === 'item') {
            // Items usually sit on floor, verify size
        ...
        const horizon = (canvas.height / 2) + player.pitch; // <--- MODIFIED
...
function renderPlayerHands() {
   ...
   // --- CONFIGURATION ---
    const scale = 0.9; // How big the weapon is (0.7 = 70% of screen width)
    ...
    // You might also want to adjust Y to lower/raise the hand
    //const y = canvas.height - handHeight * 0.95; (stable version)
    const y = (canvas.height - handHeight * 0.95) + (player.pitch * 0.5); //offset version
    ...
    if (enemy.attackElapsed >= attack.windup && 
        ...
        if (dist <= attack.range && !player.invincible) {
            ...
            if (!wasBlocked) {
                ...
                // Apply damage
                player.health -= attack.damage; // Or enemy.damage based on your code
                savePlayerState();
...
function renderShield() {
    ...
    const scale = 0.55; // Slightly smaller than screen width
    ...
    const y = (canvas.height - drawHeight * 0.8) + (player.pitch * 0.5) + yOffset;
