...
for (let y = 0; y < map.length; y++) {
    for (let x = 0; x < map[y].length; x++) {
        if (map[y][x] === 3) {    //This is where you add npcs
            let npcDialogue;
            if (x === 2 && y === 2) {
                npcDialogue = ["I'm guarding this place.", "Be careful!"];
            } else if (x === 1 && y === 5) {
                npcDialogue = ["I love this dungeon.", "It's so mysterious!"];
            } else {
                npcDialogue = ["Hello!", "Nice to see you."]; // Default dialogue
            }
...
function checkNPCInteraction() {
    objects.forEach(obj => {
        ...
        if (distance < 50) {  // Adjust range as needed
