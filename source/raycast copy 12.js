...
const lightSource = {
    x: 200, // Light position (set dynamically in-game)
    y: 200,
    intensity: 100 // Light range
};
...
function render3D(rays) {
    ...
    rays.forEach((ray, i) => {
        ...
        let brightness = Math.max(0.2, 1 - ray.distance / MAX_DEPTH); // 1.0 (near) to 0.2 (far)
        // Get the base color from the map
let baseColor = wallColors[ray.wallType] || "rgb(255, 255, 255)"; // Default to white if unknown
...
ctx.fillStyle = `rgb(${r}, ${g}, ${b})`; // Apply shading
...
function renderObjects() {
    ...
    if (isBlocked) return;
        ...
        const lightDist = Math.sqrt((obj.x - lightSource.x) ** 2 + (obj.y - lightSource.y) ** 2);
        const lightFactor = Math.max(0.2, 1 - lightDist / lightSource.intensity); // 1.0 near light, 0.2 far
        ctx.globalAlpha = lightFactor; // Make far objects dimmer
