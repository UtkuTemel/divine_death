...
function castRays() {
    ...
    for (let i = 0; i < NUM_RAYS; i++) {
        ...
        while (!hitFullWall && distAccumulator < effectiveMaxDepth / TILE_SIZE) {
            let side = 0;
...
function render3D(rays) {
    ...
    rays.forEach((ray, i) => {
        ...
        for (let j = ray.steps.length - 1; j >= 0; j--) {
            ...
            if (hit.distance >= effectiveMaxDepth) continue;
...
function renderFloorAndCeilingFast() {
    ...
    // --- FLOOR RENDER LOOP ---
    // Start from the Horizon and go DOWN to the bottom of the canvas
    for (let y = Math.floor(horizon); y < canvas.height; y += FLOOR_QUALITY) {
        ...
        // Prevent division by zero at the exact horizon line
        if (currentPixelRow <= 0) continue; 

        // Math to calculate distance based on pitch
        const rowDistance = (playerEyeHeight * PROJECTION_PLANE_DIST) / currentPixelRow;

        if (rowDistance >= effectiveMaxDepth) continue;

        // --- Lighting Calculation (Preserved your logic) ---
        ...
        for (let x = 0; x < reducedWidth; x++) {
            // --- NEW: Calculate brightness based on the specific floor pixel ---
            let pixelBrightness = calculateIllumination(floorX, floorY, rowDistance);
            ...
            // Add boundary checks to prevent flickering if math goes slightly out of bounds
            if(fx >= 0 && fy >= 0 && fx < floorTextureSize && fy < floorTextureSize) {
                const fIdx = (fy * floorTextureSize + fx) * 4;
                const p = x * 4;
                pixels[p] = Math.min(255, Math.floor(floorTex.data[fIdx] * pixelBrightness));
                pixels[p + 1] = Math.min(255, Math.floor(floorTex.data[fIdx + 1] * pixelBrightness));
                pixels[p + 2] = Math.min(255, Math.floor(floorTex.data[fIdx + 2] * pixelBrightness));
                pixels[p + 3] = 255;
            }
            floorX += floorStepX;
            floorY += floorStepY;
    ...
    // --- CEILING RENDER LOOP ---
    // Start from TOP of screen (0) and go DOWN to the Horizon
    // We use Math.min to ensure we don't loop past the canvas if looking way up
    const ceilingEnd = Math.min(canvas.height, Math.floor(horizon));

    for (let y = 0; y < ceilingEnd; y += FLOOR_QUALITY) {
        
        // Distance from the horizon line (inverted for ceiling)
        ...
        for (let x = 0; x < reducedWidth; x++) {
            // --- NEW: Calculate brightness based on the specific ceiling pixel ---
            let pixelBrightness = calculateIllumination(ceilX, ceilY, rowDistance);
            ...
            if(cx >= 0 && cy >= 0 && cx < ceilTextureSize && cy < ceilTextureSize) {
                ...
                const p = x * 4;
                pixels[p] = Math.min(255, Math.floor(ceilTex.data[cIdx] * pixelBrightness));
                pixels[p + 1] = Math.min(255, Math.floor(ceilTex.data[cIdx + 1] * pixelBrightness));
                pixels[p + 2] = Math.min(255, Math.floor(ceilTex.data[cIdx + 2] * pixelBrightness));
                pixels[p + 3] = 255;
            }
            ceilX += floorStepX;
            ceilY += floorStepY;
...
function renderSpriteLayer() {
    ...
    renderList.forEach(item => {
        ...
        if (distance >= effectiveMaxDepth) return;
    ...
    if (enemy.currentHealth <= 0) {
        ...
        if (enemy.state === "chasing") {
            if (enemy.memoryTimer <= 0) {
                ...
                const targetX = hasLOS ? player.x : enemy.lastSeenX;
                const targetY = hasLOS ? player.y : enemy.lastSeenY;
