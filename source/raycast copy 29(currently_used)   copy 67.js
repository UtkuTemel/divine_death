...
function renderSpriteLayer() {
    ...
    // --- LIGHTING CONSTANTS (Copied from render3D for consistency) ---
    const MAX_LIGHT_DISTANCE = 500;
    const MIN_BRIGHTNESS = 0.3;
    const MAX_BRIGHTNESS = 1.0;
    ...
    renderList.forEach(item => {
        ...
        let brightness = MIN_BRIGHTNESS + 
            (MAX_BRIGHTNESS - MIN_BRIGHTNESS) * (1 - Math.min(1, distance / MAX_LIGHT_DISTANCE));
        brightness = Math.pow(brightness, 1.5);
    ...
    if (enemy.currentHealth <= 0) {
        ...
        if (enemy.state === "chasing") {
            if (enemy.memoryTimer <= 0) {
                ...
                // Only tick down the memory timer if they CANNOT see the player
                if (!hasLOS) {
                    enemy.memoryTimer -= deltaTime * 1000;
                }

                // Chase either the player's current position (if seen) or last seen position
                ...
                // If they lost LOS and reach the exact spot they last saw you, 
                // they get confused and their memory drains much faster (giving up)
                if (!hasLOS && distToTarget < 20) {
                    ...
                    if (!movedThisFrame && isEnemyNearWall(enemy)) {
                        tryPathAroundWall(enemy, targetX, targetY, deltaTime);
                    }
        ...
        if (enemy.state === "chasing") {
            if (enemy.memoryTimer <= 0) {
                ...
                if (!hasLOS && distToTarget < 20) {
                    ...
                    // JUST move with collision, no extra pathing logic needed!
                    moveEnemyWithCollision(enemy, moveX, moveY);
                    movedThisFrame = (enemy.x !== prevX || enemy.y !== prevY);
                }
            }
...
function moveEnemyWithCollision(enemy, dx, dy) {
    ...
    enemy.x += dx;
    if (isEnemyInWall(enemy)) {
        enemy.x = originalX; // Hit a wall horizontally, revert X only
    }

    // 2. Move Y and check collision (SLIDING LOGIC)
    enemy.y += dy;
    if (isEnemyInWall(enemy)) {
        enemy.y = originalY; // Hit a wall vertically, revert Y only
    }
