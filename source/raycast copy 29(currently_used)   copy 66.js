    ...
    currentEnemies.forEach(other => {
        if (other !== enemy && other.currentHealth > 0) {
            ...
            if (dist < ENEMY_COLLISION_RADIUS * 2) {
                ...
                // Apply push, but check walls so they don't push each other out of bounds
                enemy.x += nx * overlap * 0.3;
                if (isEnemyInWall(enemy)) enemy.x -= nx * overlap * 0.3;
                
                enemy.y += ny * overlap * 0.3;
                if (isEnemyInWall(enemy)) enemy.y -= ny * overlap * 0.3;
            }
        }
    });
}
