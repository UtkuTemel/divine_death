...
function getAbsoluteMaxDepth() {
    ...
    // Check if any shining projectile is flying past the player's view distance
    for (let i = 0; i < projectiles.length; i++) {
        if (projectiles[i].shining) {
            let reach = Math.hypot(player.x - projectiles[i].x, player.y - projectiles[i].y) + projectiles[i].lightRadius;
            if (reach > maxD) maxD = reach;
        }
    }
    
    // Check if any burning enemies are illuminating the distance
    for (let i = 0; i < currentEnemies.length; i++) {
        const enemy = currentEnemies[i];
        if (enemy.state !== "DEAD" && enemy.activeEffects) {
            for (const effect of enemy.activeEffects) {
                const def = STATUS_EFFECTS[effect.id];
                if (def && def.shining) {
                    let reach = Math.hypot(player.x - enemy.x, player.y - enemy.y) + def.lightRadius;
                    if (reach > maxD) maxD = reach;
                }
            }
        }
...
function isPointVisible(targetX, targetY, distToPlayer) {
    // If it's within the player's raw vision, it's visible
    if (distToPlayer <= currentMaxDepth + depthFlickerOffset) return true;
    
    // Otherwise, check if it's illuminated by a projectile
    for (let i = 0; i < projectiles.length; i++) {
        let p = projectiles[i];
        if (p.shining && Math.hypot(targetX - p.x, targetY - p.y) <= p.lightRadius) {
            return true;
        }
    }
    
    // Or illuminated by a status effect on an enemy
    for (let i = 0; i < currentEnemies.length; i++) {
        const enemy = currentEnemies[i];
        if (enemy.state !== "DEAD" && enemy.activeEffects) {
            for (const effect of enemy.activeEffects) {
                const def = STATUS_EFFECTS[effect.id];
                if (def && def.shining && Math.hypot(targetX - enemy.x, targetY - enemy.y) <= def.lightRadius) {
                    return true;
                }
            }
        }
    }
    return false; // Cloaked in darkness
}
...
function calculateIllumination(targetX, targetY, playerDistance) {
    ...
    // 1. Base brightness from Player
    let maxBrightness = MIN_BRIGHTNESS +
        (MAX_BRIGHTNESS - MIN_BRIGHTNESS) * (1 - Math.min(1, playerDistance / effectiveMaxLight));
    maxBrightness = Math.pow(maxBrightness, 1.5);

    // 2. Check all active shining projectiles
    for (let i = 0; i < projectiles.length; i++) {
        const p = projectiles[i];
        if (p.shining) {
            const distToProj = Math.hypot(targetX - p.x, targetY - p.y);
            
            if (!isNaN(distToProj) && distToProj < p.lightRadius) {
                let projBrightness = MIN_BRIGHTNESS + 
                    (MAX_BRIGHTNESS - MIN_BRIGHTNESS) * (1 - Math.min(1, distToProj / p.lightRadius));
                projBrightness = Math.pow(projBrightness, 1.5);
                
                if (projBrightness > maxBrightness) {
                    maxBrightness = projBrightness;
                }
            }
        }
    }

    // 3. Check enemies with shining status effects
    for (let i = 0; i < currentEnemies.length; i++) {
        const enemy = currentEnemies[i];
        if (enemy.state !== "DEAD" && enemy.activeEffects) {
            for (const effect of enemy.activeEffects) {
                const def = STATUS_EFFECTS[effect.id];
                if (def && def.shining) {
                    const distToEnemy = Math.hypot(targetX - enemy.x, targetY - enemy.y);
                    if (!isNaN(distToEnemy) && distToEnemy < def.lightRadius) {
                        let enemyBrightness = MIN_BRIGHTNESS + 
                            (MAX_BRIGHTNESS - MIN_BRIGHTNESS) * (1 - Math.min(1, distToEnemy / def.lightRadius));
                        enemyBrightness = Math.pow(enemyBrightness, 1.5);
                        
                        if (enemyBrightness > maxBrightness) {
                            maxBrightness = enemyBrightness;
                        }
                    }
                }
            }
    ...
    return Math.min(MAX_BRIGHTNESS, maxBrightness);
}
...
function render3D(rays) {
    ...
    // Depth buffer stores the raw distance to the wall for every column
    const depthBuffer = new Float32Array(NUM_RAYS).fill(Infinity); 
    ...
    rays.forEach((ray, i) => {
       ...
       for (let j = ray.steps.length - 1; j >= 0; j--) {
            ...
            // Skip rendering if it's deep in the dark and no fireball is near it
            if (!isPointVisible(hit.exactHitX, hit.exactHitY, hit.distance)) continue;
            ...
            // Lighting
            let brightness = calculateIllumination(hit.exactHitX, hit.exactHitY, correctedDistance);
            if (hit.side === 1) brightness *= 0.85; 
            ...
            if (wallTex) {
                ...
                ctx.drawImage(wallTex, texX * wallTex.width, 0, 1, wallTex.height, i * columnWidth, wallTop, columnWidth, wallHeight);
            } else {
            ...
            if (isDoorFace) {
                ...
                if (wallX < doorStart || wallX > doorEnd) {
                    ...
                    const topFrameHeight = doorTop - wallTop;
                    const bottomFrameY = doorTop + doorDisplayHeight;
                    const bottomFrameHeight = (wallTop + wallHeight) - bottomFrameY;
                    if (topFrameHeight > 0) ctx.fillRect(i * columnWidth, wallTop, columnWidth, topFrameHeight);
                    if (bottomFrameHeight > 0) ctx.fillRect(i * columnWidth, bottomFrameY, columnWidth, bottomFrameHeight);
                }
            } else {
                ctx.fillRect(i * columnWidth, wallTop, columnWidth, wallHeight);
            }
            ...
            if (isDoorFace) { 
                ...
                if (wallX >= doorStart && wallX <= doorEnd) {
                    ...
                    if (doorTex && doorTex.complete) {
                        ctx.drawImage(doorTex, doorTexX * doorTex.width, 0, 1, doorTex.height, i * columnWidth, doorTop, columnWidth, doorDisplayHeight);
                        ctx.fillStyle = `rgba(0, 0, 0, ${1 - brightness})`;
                        ctx.fillRect(i * columnWidth, doorTop, columnWidth, doorDisplayHeight);
                    }
                }
            }
        }
...
function renderFloorAndCeilingFast() {
    ...
    // --- FLOOR RENDER LOOP ---
    // ==========================================
    for (let y = Math.floor(horizon); y < canvas.height; y += FLOOR_QUALITY) {
        ...
        for (let x = 0; x < reducedWidth; x++) {
            ...
            let pixelBrightness = playerBrightness;

            // 1. Check Projectile Lights
            for (let i = 0; i < projectiles.length; i++) {
                const p = projectiles[i];
                if (!p.shining) continue;

                const dx = Math.abs(floorX - p.x);
                const dy = Math.abs(floorY - p.y);
                if (dx > p.lightRadius || dy > p.lightRadius) continue;

                const distSq = dx * dx + dy * dy;
                const radiusSq = p.lightRadius * p.lightRadius;
                
                if (distSq <= radiusSq) {
                    isVisible = true; 
                    let pBright = MIN_BRIGHTNESS + (MAX_BRIGHTNESS - MIN_BRIGHTNESS) * (1 - Math.sqrt(distSq) / p.lightRadius);
                    pBright = Math.pow(pBright, 1.5);
                    if (pBright > pixelBrightness) pixelBrightness = pBright;
                }
            }

            // 2. NEW: Check Burning Enemy Lights (Using floorX/floorY)
            for (let i = 0; i < currentEnemies.length; i++) {
                const enemy = currentEnemies[i];
                if (enemy.state !== "DEAD" && enemy.activeEffects) {
                    for (const effect of enemy.activeEffects) {
                        const def = STATUS_EFFECTS[effect.id];
                        if (def && def.shining) {
                            const dx = Math.abs(floorX - enemy.x); 
                            const dy = Math.abs(floorY - enemy.y);
                            if (dx > def.lightRadius || dy > def.lightRadius) continue;

                            const distSq = dx * dx + dy * dy;
                            const radiusSq = def.lightRadius * def.lightRadius;
                            
                            if (distSq <= radiusSq) {
                                isVisible = true;
                                let eBright = MIN_BRIGHTNESS + (MAX_BRIGHTNESS - MIN_BRIGHTNESS) * (1 - Math.sqrt(distSq) / def.lightRadius);
                                eBright = Math.pow(eBright, 1.5);
                                if (eBright > pixelBrightness) pixelBrightness = eBright;
                            }
                        }
                    }
                }
            ...
            if (!isVisible) {
                ...
                if(fx >= 0 && fy >= 0 && fx < floorTextureSize && fy < floorTextureSize) {
                    ...
                    const pIdx = x * 4;
                    pixels[pIdx] = Math.min(255, Math.floor(floorTex.data[fIdx] * pixelBrightness));
                    pixels[pIdx + 1] = Math.min(255, Math.floor(floorTex.data[fIdx + 1] * pixelBrightness));
                    pixels[pIdx + 2] = Math.min(255, Math.floor(floorTex.data[fIdx + 2] * pixelBrightness));
                    pixels[pIdx + 3] = 255;
                }
            }
            floorX += floorStepX;
    ...
    for (let y = 0; y < ceilingEnd; y += FLOOR_QUALITY) {
        ...
        for (let x = 0; x < reducedWidth; x++) {
            ...
            for (let i = 0; i < projectiles.length; i++) {
                ...
                const dx = Math.abs(ceilX - p.x);
                const dy = Math.abs(ceilY - p.y);
                if (dx > p.lightRadius || dy > p.lightRadius) continue;
            ...
            // 2. NEW: Check Burning Enemy Lights (Using ceilX/ceilY)
            for (let i = 0; i < currentEnemies.length; i++) {
                const enemy = currentEnemies[i];
                if (enemy.state !== "DEAD" && enemy.activeEffects) {
                    for (const effect of enemy.activeEffects) {
                        const def = STATUS_EFFECTS[effect.id];
                        if (def && def.shining) {
                            const dx = Math.abs(ceilX - enemy.x); 
                            const dy = Math.abs(ceilY - enemy.y);
                            if (dx > def.lightRadius || dy > def.lightRadius) continue;
            ...
            if (!isVisible) {
                ...
                if (cx >= 0 && cy >= 0 && cx < ceilTextureSize && cy < ceilTextureSize) {
                    ...
                    const pIdx = x * 4;
                    pixels[pIdx] = Math.min(255, Math.floor(ceilTex.data[cIdx] * pixelBrightness));
                    pixels[pIdx + 1] = Math.min(255, Math.floor(ceilTex.data[cIdx + 1] * pixelBrightness));
                    pixels[pIdx + 2] = Math.min(255, Math.floor(ceilTex.data[cIdx + 2] * pixelBrightness));
                    pixels[pIdx + 3] = 255;
                }
            }
            ceilX += ceilStepX;
...
function renderSpriteLayer() {
    ...
    renderList.forEach(item => {
        ...
        let brightness = calculateIllumination(obj.x, obj.y, distance);
        ...
        if (sprite && sprite.complete) {
            // Apply brightness filter based on distance
            ctx.filter = `brightness(${brightness})`; 
            ctx.drawImage(sprite, screenX - totalWidth / 2, screenY, totalWidth, totalHeight);
            // Reset filter immediately so UI elements don't get dark
            ctx.filter = "none"; 
        } else {
            // Debug Fallback
...
function renderDialogue() {
    ...
    if (Math.floor(Date.now() / 500) % 2 === 0) {
        ctx.font = "12px 'Press Start 2P'";
        ctx.textAlign = "right";
        ctx.fillText("Press F ▼", canvas.width - 70, boxY + boxHeight - 15);
    }
}
