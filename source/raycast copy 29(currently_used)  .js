...
function loadLevel(levelName, playerState = null) {
    ...
    wallObjects = [...(level.wallObjects || [])];
    wallObjects.forEach(obj => {
        obj.initialized = false; // Add a flag
        if (obj.type === "door") {
            // Initialize door properties if not already set by persistence later
            if (obj.state === undefined) {
                 obj.state = obj.locked ? DOOR_STATES.LOCKED : DOOR_STATES.CLOSED; // Start locked if specified
                 obj.animationProgress = (obj.state === DOOR_STATES.OPEN) ? 1 : 0; // Start open if needed (though unlikely from definition)
            }
             // Ensure locked status from definition is respected initially
            if (obj.locked === undefined && level.wallObjects.find(def => def.mapX === obj.mapX && def.mapY === obj.mapY && def.type === 'door')?.locked) {
                 obj.locked = true;
                 obj.state = DOOR_STATES.LOCKED;
            } else if (obj.locked === undefined) {
                 obj.locked = false;
            }
            obj.initialized = true;
        } else if (obj.type === "torch") {
             // Add any specific torch initialization if needed
             obj.initialized = true;
        } else if (obj.type === "painting") {
             // Add any specific painting initialization if needed
             obj.initialized = true;
        }
...
function castRays() {
    ...
    for (let i = 0; i < NUM_RAYS; i++) {
        ...
        let hit = false;
        let wallType = 0;
        let exactHitX = player.x; // Initialize here
        let exactHitY = player.y; // Initialize here

        while (!hit && dist < MAX_DEPTH) {
            dist += RAY_STEP_SIZE;
            
            const currentX = player.x + cosRayAngle * dist;
            const currentY = player.y + sinRayAngle * dist;
            ...
            if (mapY >= 0 && mapY < map.length && mapX >= 0 && mapX < map[mapY].length) {
                ...
                // FIX: Include animated walls in collision detection
                if (cellValue > 0 && cellValue !== 3) {
                 ...
                 // Ray went out of bounds - treat as max depth hit but store last valid coords
                 if (dist >= MAX_DEPTH) {
                    exactHitX = currentX;
                    exactHitY = currentY;
                 }
       ...
       // If no wall hit within MAX_DEPTH, ensure coords reflect max distance
       if (!hit) {
           exactHitX = player.x + cosRayAngle * MAX_DEPTH;
           exactHitY = player.y + sinRayAngle * MAX_DEPTH;
       }
        ...
        rays.push({
            ...
            distance: dist,
            wallType,
            exactHitX, // Use stored final coordinates
            exactHitY, // Use stored final coordinates
        });
    }
    return rays;
}
...
function render3D(rays) {
    ...
    ctx.drawImage(
        currentFrame,
        wallX * currentFrame.width, 0, 1, currentFrame.height,
        i * columnWidth, wallTop, columnWidth, wallHeight
    );
...
// Fix 1: Update the renderWallObjects function to properly handle wall heights
function renderWallObjects(rays) {
    const objectsToRender = [];
    const columnWidth = canvas.width / NUM_RAYS;

    // 1. Calculate screen properties for each wall object
    wallObjects.forEach(objPos => {
        const objDef = WALL_OBJECTS[objPos.type];
        if (!objDef) return;

        // --- Calculate World Position ---
        let worldX, worldY;
        const centerOffset = TILE_SIZE / 2;
        const placementOffset = 0.5; // Place slightly off the wall surface

        switch(objPos.wallFace) {
            case 'north':
                worldX = objPos.mapX * TILE_SIZE + centerOffset;
                worldY = objPos.mapY * TILE_SIZE + placementOffset;
                break;
            case 'south':
                worldX = objPos.mapX * TILE_SIZE + centerOffset;
                worldY = (objPos.mapY + 1) * TILE_SIZE - placementOffset;
                break;
            case 'west':
                worldX = objPos.mapX * TILE_SIZE + placementOffset;
                worldY = objPos.mapY * TILE_SIZE + centerOffset;
                break;
            case 'east':
                worldX = (objPos.mapX + 1) * TILE_SIZE - placementOffset;
                worldY = objPos.mapY * TILE_SIZE + centerOffset;
                break;
        ...
        let angleDiff = angleToObject - player.angle;

        // Normalize angle difference
        while (angleDiff < -Math.PI) angleDiff += 2 * Math.PI;
        while (angleDiff > Math.PI) angleDiff -= 2 * Math.PI;

        // Rough cull based on angle (wider than FOV)
        if (Math.abs(angleDiff) > FOV) return;

        // --- Project onto Screen ---
        // Use tan for projection, correct for fisheye
        const correctedDistance = distance * Math.cos(angleDiff);
        const screenX = canvas.width / 2 + Math.tan(angleDiff) * (canvas.height / 2); // Perspective projection


        // --- Calculate Size and Vertical Position ---
        // Get the height multiplier of the *actual wall tile* the object is on
        const wallTileType = map[objPos.mapY]?.[objPos.mapX];
        if (!wallTileType || wallTileType === 0 || wallTileType === 3) return; // Don't render on empty space or non-solid walls
        
        const wallHeightMultiplier = wallHeights[wallTileType] || 1.0;
        const projectedWallHeight = Math.min(30000 / correctedDistance, canvas.height) * wallHeightMultiplier;
        const objectRenderSize = projectedWallHeight * (objDef.size || 0.5); // Use definition size

        // Calculate vertical position based on wall height and object's yPos (0=top, 1=bottom)
        const wallTopScreen = (canvas.height - projectedWallHeight) / 2;
        const objectTopScreen = wallTopScreen + projectedWallHeight * (objDef.yPos || 0.5) - objectRenderSize / 2; // Center vertically based on yPos

        objectsToRender.push({
            objPos,
            objDef,
            distance: correctedDistance, // Use corrected distance for depth sorting
            screenX,
            screenY: objectTopScreen,
            size: objectRenderSize,
            wallHeight: projectedWallHeight // Store wall height for reference if needed
        });
    });

    // 2. Sort objects by distance (far to near)
    objectsToRender.sort((a, b) => b.distance - a.distance);

    // 3. Render Objects with Occlusion Check
    objectsToRender.forEach(objData => {
        const { objPos, objDef, distance, screenX, screenY, size } = objData;

        // --- Precise Occlusion Check ---
        const startX = screenX - size / 2;
        const endX = screenX + size / 2;
        const startColumn = Math.max(0, Math.floor(startX / columnWidth));
        const endColumn = Math.min(NUM_RAYS - 1, Math.floor(endX / columnWidth));

        let isVisible = false;
        for (let i = startColumn; i <= endColumn; i++) {
            // Check if this part of the object is closer than the wall in this column
            // Add a small buffer (e.g., 2) to prevent z-fighting
            if (distance < rays[i].distance + 2) {
                isVisible = true;
                break;
            }
        }

        if (!isVisible) return; // Object is fully occluded

        // --- Get Correct Sprite ---
        let sprite = null;
        if (objPos.type === 'torch') {
            if (objDef.loaded && objDef.images[objDef.currentFrame]?.complete) {
                sprite = objDef.images[objDef.currentFrame];
            }
        } else if (objPos.type === 'door') {
             if (!objDef.loaded) return; // Skip if door images aren't loaded

            if (objPos.locked && objPos.state === DOOR_STATES.CLOSED) { // Check instance lock state FIRST
                 sprite = objDef.images[DOOR_STATES.LOCKED];
            } else if (objPos.state === DOOR_STATES.OPEN || objPos.state === DOOR_STATES.OPENING) {
                 sprite = objDef.images[DOOR_STATES.OPEN];
            } else { // CLOSED and UNLOCKED
                 sprite = objDef.images[DOOR_STATES.CLOSED];
            }

            // Fallback if a specific state image failed to load
            if (!sprite?.complete) sprite = objDef.images[DOOR_STATES.CLOSED] || null;

        } else if (objPos.type === 'painting') {
            if (objDef.image?.complete) {
                sprite = objDef.image;
            }
        }
        // Add other types here

        if (!sprite || !sprite.complete) return; // Skip if sprite not loaded/found

        // --- Calculate Sliding Animation (Doors) ---
        let animationXOffset = 0;
        if (objPos.type === 'door' && (objPos.state === DOOR_STATES.OPENING || objPos.state === DOOR_STATES.CLOSING)) {
             // Slide horizontally based on progress, relative to object size
             // Adjust direction based on wall face if needed (e.g., slide up/down for east/west doors)
             if (objPos.wallFace === 'north' || objPos.wallFace === 'south') {
                 animationXOffset = size * 0.8 * objPos.animationProgress;
             } else { // east/west - door slides vertically - we adjust Y instead
                 // This requires modifying screenY calculation slightly just for doors
                 // Let's stick to horizontal slide for simplicity now
                 animationXOffset = size * 0.8 * objPos.animationProgress;
             }
        }
        ...
        ctx.drawImage(
            sprite,
            screenX - size / 2 + animationXOffset, // Apply animation offset here
            screenY,
            size,
            size
        );
...
function renderDoor(doorObj, ray, columnIndex, columnWidth, renderedWallObjects, objKey) {
    ...
    if (!doorImage || !doorImage.complete) {
        ...
        ctx.fillRect(
            columnIndex * columnWidth - doorSize / 2,
            yPos - doorSize,
            doorSize,
            doorSize
        );
    ...
    ctx.drawImage(
        doorImage,
        columnIndex * columnWidth + finalXOffset - doorSize / 2,
        yPos - doorSize,
        doorSize,
        doorSize
    );
