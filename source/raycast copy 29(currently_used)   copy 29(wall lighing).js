...
const DOOR_ANIMATION_SPEED = 0.05; // How fast doors open/close
...
const ANIMATED_WALLS = {
    ...
    10: { // Another animated wall type
        frames: [new Image(), new Image()], // 2-frame animation  
        frameCount: 2,
        frameDelay: 15,
        currentFrame: 0,
        frameCounter: 0,
...
ANIMATED_WALLS[10].frames[0].src = "wall_anim2_frame1.png";
ANIMATED_WALLS[10].frames[1].src = "wall_anim2_frame2.png";
...
function castRays() {
    ...
    for (let i = 0; i < NUM_RAYS; i++) {
        ...
        let wallType = 0;
        let isDoor = false;
        let doorState = DOOR_TYPES.CLOSED;
        let doorProgress = 1.0; // 1.0 = fully closed, 0.0 = fully open
        ...
        while (!hit && dist < MAX_DEPTH) {
            ...
            if (mapY >= 0 && mapY < map.length && mapX >= 0 && mapX < map[mapY].length) {
                ...
                // Check if this is a door (negative values could represent doors)
                if (cellValue < 0) {
                    isDoor = true;
                    wallType = Math.abs(cellValue); // Use absolute value for texture
                    doorState = getDoorState(mapX, mapY);
                    doorProgress = getDoorProgress(mapX, mapY);
                    hit = true;
                }
                // Regular walls
...
function render3D(rays) {
    ...
    rays.forEach((ray, i) => {
        ...
        // Handle doors differently
        if (ray.isDoor) {
            renderDoor(ray, i, columnWidth);
            return;
        }
 ...
 rays.forEach((ray, i) => {
    ...
    for (const objPos of wallObjects) {
        ...
        if (objPos.type === "torch") {
            ...
            // Use the actual wall height for this position
...
function renderDoor(ray, columnIndex, columnWidth) {
    const heightMultiplier = wallHeights[ray.wallType] || 1.0;
    const baseWallHeight = Math.min(30000 / ray.distance, canvas.height);
    
    // Adjust height based on door opening progress
    const doorHeight = baseWallHeight * heightMultiplier * ray.doorProgress;
    const wallTop = (canvas.height - doorHeight) / 2;
    
    const texture = wallTextures[ray.wallType];
    if (!texture || !texture.complete) {
        // Fallback: render as colored door
        ctx.fillStyle = ray.doorState === DOOR_TYPES.LOCKED ? "rgb(100, 0, 0)" : "rgb(100, 50, 0)";
        ctx.fillRect(columnIndex * columnWidth, wallTop, columnWidth, doorHeight);
        return;
    }

    // Calculate texture coordinates (similar to regular walls)
    const hitX = player.x + Math.cos(ray.angle) * ray.distance;
    const hitY = player.y + Math.sin(ray.angle) * ray.distance;
    ...
    if (relX < 2 || relX > TILE_SIZE - 2) {
        wallX = relX < 2 ? (relY / TILE_SIZE) : (1 - relY / TILE_SIZE);
    } else {
        wallX = relY < 2 ? (relX / TILE_SIZE) : (1 - relX / TILE_SIZE);
    }

    // Apply door opening effect by adjusting texture rendering
    const textureYStart = (1 - ray.doorProgress) * texture.height / 2;
    const textureHeight = texture.height * ray.doorProgress;

    ctx.drawImage(
        texture,
        wallX * texture.width, textureYStart, 1, textureHeight,
        columnIndex * columnWidth, wallTop, columnWidth, doorHeight
    );

    // Add lighting
    const brightness = 0.6 + (0.4 * ray.doorProgress); // Doors get darker as they open
    ctx.fillStyle = `rgba(0, 0, 0, ${1 - brightness})`;
    ctx.fillRect(columnIndex * columnWidth, wallTop, columnWidth, doorHeight);
}
function checkDoorInteraction() {
    // Check in front of player for doors
    ...
    if (mapY >= 0 && mapY < map.length && mapX >= 0 && mapX < map[mapY].length) {
        if (map[mapY][mapX] < 0) { // This is a door
            const doorState = getDoorState(mapX, mapY);
            
            // Show interaction prompt
            ctx.font = "16px 'Press Start 2P'";
            ctx.fillStyle = "white";
            ctx.textAlign = "center";
            ctx.fillText("Press F to open/close door", canvas.width/2, canvas.height - 80);
            
            // Handle interaction
            if (keys["KeyF"] && !showDialogue) {
                toggleDoor(mapX, mapY);
...
function getDoorKey(mapX, mapY) {
    return `${mapX},${mapY}`;
}

function getDoorState(mapX, mapY) {
    const key = getDoorKey(mapX, mapY);
    return doorStates.get(key)?.state || DOOR_TYPES.CLOSED;
}

function getDoorProgress(mapX, mapY) {
    const key = getDoorKey(mapX, mapY);
    return doorStates.get(key)?.progress || 1.0;
}

function setDoorState(mapX, mapY, state) {
    const key = getDoorKey(mapX, mapY);
    if (!doorStates.has(key)) {
        doorStates.set(key, { state, progress: 1.0 });
    } else {
        doorStates.get(key).state = state;
    }
}

function updateDoorProgress(mapX, mapY) {
    const key = getDoorKey(mapX, mapY);
    const door = doorStates.get(key);
    if (!door) return;

    if (door.state === DOOR_TYPES.OPENING) {
        door.progress -= DOOR_ANIMATION_SPEED;
        if (door.progress <= 0) {
            door.progress = 0;
            door.state = DOOR_TYPES.OPEN;
            // Remove collision when fully open
            map[mapY][mapX] = 0;
        }
    } else if (door.state === DOOR_TYPES.CLOSING) {
        door.progress += DOOR_ANIMATION_SPEED;
        if (door.progress >= 1) {
            door.progress = 1;
            door.state = DOOR_TYPES.CLOSED;
            // Restore collision when fully closed
            map[mapY][mapX] = -Math.abs(map[mapY][mapX]); // Keep negative for door identification
        }
    }
}

function toggleDoor(mapX, mapY) {
    const currentState = getDoorState(mapX, mapY);
    
    if (currentState === DOOR_TYPES.CLOSED) {
        setDoorState(mapX, mapY, DOOR_TYPES.OPENING);
    } else if (currentState === DOOR_TYPES.OPEN) {
        setDoorState(mapX, mapY, DOOR_TYPES.CLOSING);
        // Restore wall collision immediately when starting to close
        map[mapY][mapX] = -Math.abs(map[mapY][mapX]);
    }
}

// Update all doors in the update function
function updateDoors(deltaTime) {
    for (let y = 0; y < map.length; y++) {
        for (let x = 0; x < map[y].length; x++) {
            if (map[y][x] < 0) { // This is a door
                updateDoorProgress(x, y);
            }
        }
    }
}
