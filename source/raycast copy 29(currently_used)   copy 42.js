...
function castRays() {
  ...
  for (let i = 0; i < NUM_RAYS; i++) {
    ...
    // Convert player pos to tile units (avoid mixing pixels/tile arithmetic)
    ...
    // Length of ray from one x or y-side to next in tile units
    ...
    // initial side distances (in tile units)
    if (rayDirX < 0) {
      stepX = -1;
    ...
    // DDA
    let hit = false;
    let side = 0; // 0 -> hit vertical (x) side, 1 -> hit horizontal (y) side
    let wallType = 0;

    while (!hit && mapX >= 0 && mapX < map[0].length && mapY >= 0 && mapY < map.length) {
    ...
    // If we left bounds without hit, produce an infinity-distance ray
    if (!hit) {
      rays.push({
        angle: rayAngle,
        distance: Infinity,
        ...
        rayDirX,
        rayDirY,
        perpWallDistTiles: Infinity
      });
      continue;
    }

    // Perpendicular wall distance in TILE units (positive)
    let perpWallDistTiles;
    if (side === 0) {
      // avoid division by zero; rayDirX is not zero here
      perpWallDistTiles = (mapX - posX + (1 - stepX) / 2) / (rayDirX === 0 ? 1e-9 : rayDirX);
    } else {
      perpWallDistTiles = (mapY - posY + (1 - stepY) / 2) / (rayDirY === 0 ? 1e-9 : rayDirY);
    }
    perpWallDistTiles = Math.abs(perpWallDistTiles); // make sure it's positive

    // Convert perpendicular distance to pixels for rendering convenience
    const perpWallDistPixels = perpWallDistTiles * TILE_SIZE;

    // Exact hit in world pixels (stable, built from tile-based perp distance)
    const exactHitX = player.x + perpWallDistPixels * rayDirX;
    const exactHitY = player.y + perpWallDistPixels * rayDirY;

    rays.push({
      angle: rayAngle,
      distance: perpWallDistPixels, // in pixels
      wallType,
      exactHitX,
      exactHitY,
      side,
...
function render3D(rays) {
  ...
  // FIX: Near Clipping Plane Constant (Prevents stretching)
  ...
  rays.forEach((ray, i) => {
    ...
    // Calculate the angle of the ray relative to the player's center view
    const beta = ray.angle - player.angle;
    
    // Correct the distance to prevent fisheye distortion
    let correctedDistance = ray.distance * Math.cos(beta);
    
    // FIX: Clamp distance to prevent stretching (Maintains Fix)
    correctedDistance = Math.max(MIN_DISTANCE_CLIP, correctedDistance);

    // Get wall height multiplier
    const heightMultiplier = wallHeights[ray.wallType] || 1.0;

    // Use correctedDistance for stable height calculation
    const baseWallHeight = Math.min(30000 / correctedDistance, canvas.height * 2);
    const wallHeight = baseWallHeight * heightMultiplier;

    const halfHeight = canvas.height / 2;
    const wallBottom = halfHeight + (baseWallHeight / 2);
    ...
    let brightness =
      ...
      (MAX_BRIGHTNESS - MIN_BRIGHTNESS) *
        (1 - Math.min(1, correctedDistance / MAX_LIGHT_DISTANCE));
    brightness = Math.pow(brightness, 1.5);

    
    // --- Textured Walls (Types 5, 6, 7) ---
...
if (ray.side === 0) {
  // hit vertical wall -> offset along Y (in pixels)
  wallX = ray.exactHitY;
} else {
  // hit horizontal wall -> offset along X (in pixels)
  wallX = ray.exactHitX;
}

// Convert to tile fraction, robustly (avoid negative rounding issues)
wallX = (wallX / TILE_SIZE) % 1;
if (wallX < 0) wallX += 1; // make sure it's 0..1

// Flip when necessary using rayDir (more explicit & robust)
if ((ray.side === 0 && ray.rayDirX > 0) || (ray.side === 1 && ray.rayDirY < 0)) {
  wallX = 1 - wallX;
}
wallX = Math.max(0.001, Math.min(0.999, wallX));
      ctx.drawImage(texture, wallX * texture.width, 0, 1, texture.height, i * columnWidth, wallTop, columnWidth, wallHeight);
      ...
      ctx.fillRect(i * columnWidth, wallTop, columnWidth, wallHeight);
        ...
        let wallX;
        if (ray.side === 0) { wallX = ray.exactHitY; } else { wallX = ray.exactHitX; }
        
        wallX = wallX / TILE_SIZE;
        wallX = wallX - Math.floor(wallX);
        
        if ((ray.side === 0 && Math.cos(ray.angle) > 0) || 
            (ray.side === 1 && Math.sin(ray.angle) < 0)) {
            wallX = 1 - wallX;
        }

        wallX = Math.max(0.001, Math.min(0.999, wallX));

        ctx.drawImage(currentFrame, wallX * currentFrame.width, 0, 1, currentFrame.height, i * columnWidth, wallTop, columnWidth, wallHeight);
        ctx.fillStyle = `rgba(0, 0, 0, ${1 - brightness})`;
        ctx.fillRect(i * columnWidth, wallTop, columnWidth, wallHeight);
    } 
  ...
  }); // End of Wall Rendering Pass

  // Second pass: Render wall objects (torches, paintings)
  const renderedWallObjects = new Set();

  rays.forEach((ray, i) => {
    // We must use the clamped distance here for the object's height calculation
    let correctedDistance = ray.distance * Math.cos(ray.angle - player.angle);
    correctedDistance = Math.max(MIN_DISTANCE_CLIP, correctedDistance);

    // Use raw distance for map hit check (which wall tile the ray hits)
   const rayMapX = ray.mapX;
   const rayMapY = ray.mapY;
    ...
    for (const objPos of wallObjects) {
      ...
      if (objPos.type === 'torch') {
        ...
        // Recalculate based on clamped distance for stable height
        const baseWallHeight = Math.min(30000 / correctedDistance, canvas.height * 2);
        const wallHeight = baseWallHeight * wallHeightMultiplier;

        const objectSize = wallHeight * torch.size;
        const yPos = (canvas.height - wallHeight) / 2 + wallHeight * torch.yPos;

        // --- FIX: Robust Wall Object X-Offset Calculation (Restores alignment) ---
        let wallHitCoord;
if (ray.side === 0) {
  // E/W wall -> offset measured along Y (in pixels)
  wallHitCoord = ray.exactHitY;
} else {
  // N/S wall -> offset measured along X (in pixels)
  wallHitCoord = ray.exactHitX;
}

// fractional offset along the tile in 0..1, robust
let wallXFraction = (wallHitCoord / TILE_SIZE) % 1;
if (wallXFraction < 0) wallXFraction += 1;

// center offset
let xOffset = (wallXFraction - 0.5) * columnWidth;
let screenX = i * columnWidth + xOffset - objectSize / 2;
        ...
        else if (screenX + objectSize > fadeEnd) { alpha = (canvas.width - (screenX + objectSize)) / fadeStart; }

        ctx.globalAlpha = Math.max(0, Math.min(1, alpha));

        // Clamp the position before drawing
        if (screenX < 0) screenX = 0;
        if (screenX + objectSize > canvas.width) screenX = canvas.width - objectSize;
          
        ctx.drawImage(currentFrameImg, screenX, yPos - objectSize, objectSize, objectSize);
        ctx.globalAlpha = 1.0;

        renderedWallObjects.add(objKey);
      } else if (objPos.type === 'door') {
        // Assuming renderDoor function exists
...
    // --- FIX: Unify projection constant ---
    const distToProjPlane = canvas.height / 2;
    // --- End Fix ---
...
        
        // --- FIX: Use unified projection math ---
        // This formula now matches the logic used for the walls.
        const rowDistance = (player.height * distToProjPlane) / cameraY;
        // --- End Fix ---

        if (rowDistance >= MAX_DEPTH || rowDistance <= 0) continue; // Add check for zero/negative
...
            const p = x * 4;
            pixels[p] = Math.min(255, Math.floor(floorTex.data[fIdx] * brightness));
            pixels[p + 1] = Math.min(255, Math.floor(floorTex.data[fIdx + 1] * brightness));
            pixels[p + 2] = Math.min(255, Math.floor(floorTex.data[fIdx + 2] * brightness));
            pixels[p + 3] = 255;
...
        ctx.drawImage(
           rowCanvas, 0, 0, reducedWidth, 1,
           0, y, canvas.width, targetHeight
        );
    }
    
    // --- CEILING ---
    for (let y = 0; y < halfHeight; y += FLOOR_QUALITY) {
        const cameraY = halfHeight - y;

        const cameraToCeilingDist = currentRoomHeight - player.height;
        
        // --- FIX: Use unified projection math ---
        const rowDistance = (cameraToCeilingDist * distToProjPlane) / cameraY;
        // --- End Fix ---
...
            const p = x * 4;
            pixels[p] = Math.min(255, Math.floor(ceilTex.data[cIdx] * brightness));
            pixels[p + 1] = Math.min(255, Math.floor(ceilTex.data[cIdx + 1] * brightness));
            pixels[p + 2] = Math.min(255, Math.floor(ceilTex.data[cIdx + 2] * brightness));
            pixels[p + 3] = 255;
...
        ctx.drawImage(
          rowCanvas, 0, 0, reducedWidth, 1,
          0, y, canvas.width, FLOOR_QUALITY
        );
    }
