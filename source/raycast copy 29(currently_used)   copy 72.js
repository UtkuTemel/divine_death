...
function calculateIllumination(targetX, targetY, playerDistance) {
    ...
    for (let i = 0; i < projectiles.length; i++) {
        const p = projectiles[i];
        if (p.shining) {
            // Distance from the wall/floor/sprite to the projectile
            ...
            if (!isNaN(distToProj) && distToProj < p.lightRadius) {
                ...
                // If the projectile illuminates this spot more than the player does, use its brightness
...
function renderFloorAndCeilingFast() {
    ...
    // --- Offscreen Buffer Setup (Kept exactly as is) ---
    ...
    // --- NEW: Calculate Horizon with Pitch ---
    // Ensure pitch exists
    ...
    for (let y = Math.floor(horizon); y < canvas.height; y += FLOOR_QUALITY) {
        ...
        // Pre-calculate base player brightness for the whole row
        ...
        for (let x = 0; x < reducedWidth; x++) {
            ...
            // Highly optimized inline projectile light check
            for (let i = 0; i < projectiles.length; i++) {
                const p = projectiles[i];
                if (!p.shining) continue;

                // Fast bounding-box check to avoid Math.sqrt overhead
                ...
                if (distSq <= radiusSq) {
                    isVisible = true; // The light pierces the darkness!
    ...
    // --- OVERLAYS (Restored your Vignette Style) ---
    
    // 1. Floor Gradient: Starts Transparent at Horizon, gets Darker at Bottom
    if (horizon < canvas.height) {
        const gradient = ctx.createLinearGradient(0, horizon, 0, canvas.height);
        gradient.addColorStop(0, 'rgba(0, 0, 0, 0)');    // Transparent at Horizon (Your style)
        gradient.addColorStop(1, 'rgba(0, 0, 0, 0.85)'); // Dark at bottom
        ctx.fillStyle = gradient;
        ctx.fillRect(0, horizon, canvas.width, canvas.height - horizon);
    }

    // 2. Ceiling Gradient: Starts Dark at Top, gets Transparent at Horizon
    if (horizon > 0) {
        const ceilingGradient = ctx.createLinearGradient(0, 0, 0, horizon);
        ceilingGradient.addColorStop(0, 'rgba(0, 0, 0, 0.85)'); // Dark at top
        ceilingGradient.addColorStop(1, 'rgba(0, 0, 0, 0)');    // Transparent at Horizon (Your style)
        ctx.fillStyle = ceilingGradient;
        ctx.fillRect(0, 0, canvas.width, horizon);
    }
    
    // 3. Seam Fix: Small black strip at horizon to hide float math errors
    ctx.fillStyle = "black";
